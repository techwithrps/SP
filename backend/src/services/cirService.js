...
async function getOperationsSummary() {
  ...
  const [gateInRes, outwardRes, dispatchRes, picklistRes, asnRes, crossRes] = await Promise.all([
    pool.request().query('SELECT TOP 15 CARGO_GATE_IN_ID, REFERENCE_NO, TRUCK_NO, DRIVER, TRANSPORTER_NAME, CONVERT(VARCHAR(19), GATE_IN_DATE, 120) as GATE_IN_DATE, CONT_NO, SEAL_NO FROM CARGO_GATE_IN ORDER BY CARGO_GATE_IN_ID DESC'),
    pool.request().query('SELECT TOP 15 VEHICLE_ID, TRUCK_NO, DRIVER_NAME, TRANSPORTER_NAME, CONT_NO, SEAL_NO, CONVERT(VARCHAR(19), GATE_OUT_DATE, 120) as GATE_OUT_DATE, REMARKS FROM VEHICLE_OUTWARD_ENTRY ORDER BY VEHICLE_ID DESC'),
    pool.request().query('SELECT TOP 15 DISPATCH_ID, DISPATCH_REF_NO, TRUCK_NO, CONT_NO, CLIENT_INVOICE_NO, DISPATCH_TEMPERATURE, CONVERT(VARCHAR(10), DISPATCH_DATE, 103) as DISPATCH_DATE FROM DISPATCH_NOTE ORDER BY DISPATCH_ID DESC'),
    pool.request().query('SELECT TOP 15 PICKLIST_ID, PICKLIST_REF_NO, CONVERT(VARCHAR(19), PICKLIST_DATE, 120) as PICKLIST_DATE, TRUCK_NO FROM PICKLIST ORDER BY PICKLIST_ID DESC'),
    pool.request().query('SELECT TOP 15 a.ASN_ID, a.ASN_NO, CONVERT(VARCHAR(10), a.ASN_DATE, 103) as ASN_DATE, a.TRUCK_NO, ISNULL(cm.CUSTOMER_NAME, CAST(a.ACCOUNT_HOLDER_ID AS VARCHAR(50))) as SUPPLIER_NAME FROM ASN a LEFT JOIN CUSTOMER_MASTER cm ON cm.CUSTOMER_ID = a.ACCOUNT_HOLDER_ID ORDER BY a.ASN_ID DESC'),
    pool.request().query('SELECT TOP 15 CS_GATE_IN_ID, CS_REF_NO, TRUCK_NO, CONT_NO, SEAL_NO, CONVERT(VARCHAR(19), GATE_IN_DATE, 120) as GATE_IN_DATE FROM CROSS_STUFFING_GATE_IN ORDER BY CS_GATE_IN_ID DESC')
  ]);
