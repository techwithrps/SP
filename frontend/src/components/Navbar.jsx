        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex items-center gap-2.5 sm:gap-3 shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-100 hover:bg-slate-200/90 border border-slate-200 text-xs font-semibold text-slate-700 transition-all active:scale-95 disabled:opacity-50 cursor-pointer shadow-2xs hover:shadow-xs"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${loading ? 'animate-spin text-[#2b1f55]' : 'text-slate-600'}`} />
              <span className="hidden sm:inline">Refresh</span>
            </button>

            {/* Admin User Capsule */}
            {currentUser && (
              <div className="flex items-center bg-slate-50 hover:bg-slate-100/90 transition-all border border-slate-200/90 rounded-2xl p-1.5 pl-3 gap-3 shadow-2xs">
